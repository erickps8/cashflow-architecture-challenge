...
public class MonthlyBudgetService : IMonthlyBudgetService
{
    ...
    public async Task<MonthlyBudget?> SetAsync(int year, int month, Guid categoryId, decimal plannedAmount)
    {
        ...
        if (budget is null)
        {
            budget = new MonthlyBudget { Year = year, Month = month, CategoryId = categoryId, PlannedAmount = plannedAmount };
    ...
    public async Task<MonthlyBudgetSummary?> GetSummaryAsync(int year, int month)
    {
        ...
        var categories = (await _categoryRepository.GetAllAsync()).Where(x => x.Type == EntryType.Debit).ToList();
        var directByCategory = entries.Where(x => x.Type == EntryType.Debit && x.CategoryId.HasValue).GroupBy(x => x.CategoryId!.Value).ToDictionary(x => x.Key, x => x.Sum(y => y.Amount));
        var cardByCategory = installments.Where(x => x.CreditCardPurchase?.CategoryId != null).GroupBy(x => x.CreditCardPurchase!.CategoryId!.Value).ToDictionary(x => x.Key, x => x.Sum(y => y.Amount));
        var budgetByCategory = budgets.ToDictionary(x => x.CategoryId, x => x.PlannedAmount);
        var categoryIds = budgetByCategory.Keys.Union(directByCategory.Keys).Union(cardByCategory.Keys).Distinct().ToList();
        var items = categoryIds.Select(categoryId =>
        {
            ...
            return new MonthlyBudgetCategorySummary { CategoryId = categoryId, CategoryName = category?.Name ?? "Categoria não encontrada", PlannedAmount = budgetByCategory.GetValueOrDefault(categoryId), ActualAmount = direct + card };
        }).OrderBy(x => x.CategoryName).ToList();
        return new MonthlyBudgetSummary { Year = year, Month = month, PlannedAmount = items.Sum(x => x.PlannedAmount), ActualAmount = items.Sum(x => x.ActualAmount), Categories = items };
    }
}
