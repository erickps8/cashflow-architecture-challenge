...
public sealed class EntryService : IEntryService
{
    ...
    public EntryService(IEntryRepository entryRepository, IOutboxMessageRepository outboxRepository, INotificator notificator, IAccountRepository accountRepository, ICategoryRepository categoryRepository)
    {
    ...
    {
        if (!await IsValidAsync(amount, type, accountId, categoryId)) return null;
        var entry = new Entry { Amount = amount, Type = type, Description = description, OccurredAt = ToUtc(occurredAt), AccountId = accountId, CategoryId = categoryId, IsRecurring = isRecurring };
    ...
    public async Task<Entry?> UpdateAsync(Guid id, decimal amount, EntryType type, string description, DateTime occurredAt, Guid? accountId = null, Guid? categoryId = null, bool isRecurring = false)
    {
        ...
        if (entry is null) { Notify("Lançamento não encontrado."); return null; }
        if (!await IsValidAsync(amount, type, accountId, categoryId)) return null;
        entry.Amount = amount;
        entry.Type = type;
    ...
    public async Task<Entry?> DeferToNextMonthAsync(Guid id)
    {
        var entry = await _entryRepository.GetByIdAsync(id);
        if (entry is null) { Notify("Lançamento não encontrado."); return null; }
        if (!entry.IsRecurring) { Notify("Somente lançamentos gerados por recorrência podem ser adiados."); return null; }

        entry.OriginalOccurredAt ??= entry.OccurredAt;
        entry.OccurredAt = entry.OccurredAt.AddMonths(1);
        _entryRepository.Update(entry);
        await _entryRepository.SaveChangesAsync();
        return entry;
    }
    ...
    {
        var entry = await _entryRepository.GetByIdAsync(id);
        if (entry is null) { Notify("Lançamento não encontrado."); return false; }
    ...
    {
        try { return await _entryRepository.GetAllAsync(); }
        catch (Exception exception) { Notify($"Erro ao consultar lançamentos: {exception.Message}"); return []; }
    }

    public Task<List<Entry>> GetByMonthAsync(int year, int month)
    {
        if (month is < 1 or > 12) return Task.FromResult(new List<Entry>());
        var start = new DateTime(year, month, 1, 0, 0, 0, DateTimeKind.Utc);
        return _entryRepository.GetByPeriodAsync(start, start.AddMonths(1));
    }

    private async Task<bool> IsValidAsync(decimal amount, EntryType type, Guid? accountId, Guid? categoryId)
    {
        ...
        if (category is null) { Notify("Category not found."); return false; }
        if (category.Type != type) { Notify("Category type must match entry type."); return false; }
        return true;
    }

    private async Task AddOutboxMessageAsync(Entry entry)
    {
        var domainEvent = new EntryCreatedEvent { EntryId = entry.Id, Amount = entry.Amount, Type = (int)entry.Type, Description = entry.Description, OccurredAt = entry.OccurredAt };
        await _outboxRepository.AddAsync(new OutboxMessage { Type = nameof(EntryCreatedEvent), Payload = JsonSerializer.Serialize(domainEvent) });
    }
