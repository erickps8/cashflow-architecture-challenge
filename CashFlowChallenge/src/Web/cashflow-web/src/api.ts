...
export type AuthResult = { token?:string; name?:string; username?:string; email?:string; requiresGroup:boolean; pendingApproval:boolean; message?:string; group?:{ id:string; name:string; role:string } };
...
const sessionNameKey='cashflow_session_name';
const sessionEmailKey='cashflow_session_email';
...
export const session={
  ...
  get name(){return localStorage.getItem(sessionNameKey)},
  get email(){return localStorage.getItem(sessionEmailKey)},
  set(token:string,state:SessionState,name?:string,email?:string){localStorage.setItem(tokenKey,token);localStorage.setItem(sessionStateKey,state);if(name)localStorage.setItem(sessionNameKey,name);if(email)localStorage.setItem(sessionEmailKey,email)},
  clear(){localStorage.removeItem(tokenKey);localStorage.removeItem(sessionStateKey);localStorage.removeItem(sessionNameKey);localStorage.removeItem(sessionEmailKey)}
};
...
function storeAuth(result:AuthResult){if(result.token){const state:SessionState=result.pendingApproval?'pending':result.requiresGroup?'group':'active';session.set(result.token,state,result.name,result.email)}return result}
...
export async function register(name:string,email:string,password:string,groupName:string){return storeAuth(await request<AuthResult>('/auth/register',{method:'POST',body:JSON.stringify({name,email,password,groupName})}))}
...
export const getGroupMembers=()=>request<{id:string;name?:string;email:string;username:string;status:string;role:string}[]>('/auth/group/members');
